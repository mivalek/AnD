...
As luck would have it, Canvas provides a fully exposed [RESTful](https://en.wikipedia.org/wiki/Representational_state_transfer "Honestly, who comes up with these names?!") API whcih means that uploading all the 500+ marked submissions so that they are only visible to their author can be a matter of seconds rather than hours. In order to access the API, we first need to set up a few things.
...
```{r}
# installing from GitHub requires eiother package remotes or devtools
...
The package only contains two functions `red_data()` and `green_data()`. They both take a canidate number as their first argument and generate a dataset for the given student.
...
Marking is done according to the follwing rubric criteria:
...
The grade descriptors for each criterion can be found (here)[https://sussex.app.box.com/s/o5nalmzuj6u26w6hh12dq2ni4bks1a37]
...
An example Rmd file corresponding to the HTML document above can be find [here](https://mivalek.github.io/adata/marking/marking_example.Rmd) (link will download the file).
...
As mentioned [above][.Rmd files to mark], there's some extra HTML/Markdown appended to the end of the studetns' .Rmd files. This is where you give general summary feedback on their performance. Any R Markdown will work normally. In post-processing, the three placeholders (<code><i>&lt;!&ndash;- THE GOOD -&ndash;&gt;</i></code>, <code><i>&lt;!&ndash;- THE BAD &ndash;-&gt;</i></code>, and <code><i>&lt;!&ndash;- RECOMMENDATIONS &ndash;-&gt;</i></code>) will get replaced by boilerplate text (see bottom of the example document <a href="#marked-html">below</a>). That means that it's enough for you just to include a few bulletpoints highlighting the report's streangths, weeknesses, and giving some general recommendations for improvement.

\ 

<a name="marked-html"></a>
Once a marked and commented .Rmd file has been knitted, it will look like the example below (you can see the full-size document with a wider layout [here](https://mivalek.github.io/adata/marking/marked_example_marked.html)). As mentioned, the in-line comments are interactive and reveal the matching rubric criterion description on hover. The boxes with partial grades per criterion are also interactive an when clicked, deveal the grade descriptor.
...
The code will then amend any marks that have been changed in moreration <span style="color:red; font-weight:bold">[ TO DO! ]</span> and upload the HTML documents as feedback attachemts to each individual student submission. Again, this may take a minute.
...
1. Folow the Box link and download the .R, .Rmd and .html files from your folder.
