...
For instance, if we want to only look at data from participants who are 19 years old or younger **OR** have brown eyes, we can do this by formulation this condition as `age > 22 | eye_col == "brown"`. The complete command will then be:
...
<div class="warn">
**Note** that when joining two or more logical expressions, *e.g.,* `age > 22` and `eye_col == "brown"` using the logical operators AND `&` and OR `|`, every constituent part must be a logical expression able to work on its own.</div>
...
The pipe, however, is most often used in its implicit form. If we don't include the dot, the pipe will push whatever is to the left of it into the function of the right of it **as its first argument**. In other words, the above can be written as:
...
First three of these four steps are exactly the same thig as what you did in the previous task. As for the plotting itself, that is the topic for next week's tutorial...
