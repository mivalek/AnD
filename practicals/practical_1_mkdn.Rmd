...
### Task 1.2 Create a new summary table
```{r task1}
...
## Task 2: Histograms
### Task 2.1: Fill in the code below 
...
### Task 2.2: Plot a histogram of the `score` variable in the `random_tib` dataset

```{r task2hist}

```

Task 2.3 **Challenge**: Try the following extra tasks:
...
It is currently `r Sys.Date() %>% weekdays()`. There are `r people` people around me, `r (blueshirt/people*100) %>% round(2)` percent of which are wearing blue. 
