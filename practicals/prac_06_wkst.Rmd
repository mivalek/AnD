...
harm <- read_csv("https://raw.githubusercontent.com/mivalek/AnD/master/data/harm_data.csv")
# inspect chunk
...
# remove cases
harm <- harm %>% filter(Age > 18 & Age < 90)
# descriptives chunk
...
Today's practical worksheet gives you more practice with preparing, visualising, and analysing data for correlation and chi-squared tests. If you are planning to write your lab report on the green study, the chi-square 
...
rate_desc <- harm %>% 
  ...
  summarise(
    n = n(),
    pain_m = mean(Pain, na.rm = T),
    pain_sd = sd(Pain, na.rm = T),
    exp_m = mean(Experience, na.rm = T),
    exp_sd = sd(Experience, na.rm = T),
    agency_m = mean(Agency, na.rm = T),
    agency_sd = sd(Agency, na.rm = T)
  )
...
harm %>% 
  filter(is.na(Gender) == FALSE) %>%
  select(Gender, Humanness) %>% 
  group_by(Gender) %>% 
  table()

# Hint: don't forget to turn into matrix first!!

harm %>% 
  filter(is.na(Gender) == FALSE) %>%
  select(Pain, Experience, Agency, Consciousness, Empathy, Attractiveness) %>% 
  as.matrix() %>% 
  rcorr()
...
harm %>%
  ggplot(aes(x = Pain, y = Agency, colour = Harm)) +
  geom_point(position = "jitter") +
  facet_wrap(~Humanness) +
  theme_bw()
...
harm %>% 
  ...
  cor.test(~ Pain + Agency, ., alternative = "two.sided", method = "pearson")

cor.test(~ Pain + Agency, ., alternative = "two.sided", method = "pearson")
```

```{r}
pairs(harm[,7:10], pch = 19)
```
