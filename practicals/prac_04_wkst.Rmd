...
I think this is a rather nice plot!
...
Next, we can visualise the age breakdown by gender choice using boxplots. To do this, you need to map each both of these variables onto one axis inside of the `aes()` function: The categorical variable belongs on the x axis and the continuous on the y axis:
...
That's it for descriptive plots. Let's build up a more interesting plots that will show us something about the relationships between the main measured variables.
...
Since both of these variables are continuous, a scatterplot is probably the best option. In `ggplot()`, we create scatterplots using `geom_pooint()`:
...
It's interesting to see that different gender categories display different patters. While participants who reported their gender as female seem to be roughly spread across the diagonal of the plot, those who reported their gender as male a seem to be distributed more flatly in the top half of the plot. Unfortunately, we do not have enough data from participants in the "Other" category to make any observations.
...
`r task()`First of all, if you haven't finished last week's tasks pick up where you left in the [week 3 practical worksheet](https://mivalek.github.io/adata/prac/prac_03_wkst.html#worksheet). There is a [version with solutions](https://mivalek.github.io/adata/prac/sol/prac_03_wkst_sol.html#worksheet) available no if you need help but when you use it, make sure you understand the code! There's no point copy-pasting the solutions just for the sake of completing the tasks.
...
```{r, echo = solution, class="solution", fig.cap="Figure 1. Relationship between pain and consciousness rating by levels of harm for human and non-human avatars."}
pain_consc
```
...
This week you learnt how to make pretty data visualisations and you added a Materials section to your write-up. Next time, we'll finally get to actually analysing data and testing hypotheses. But, for now...
